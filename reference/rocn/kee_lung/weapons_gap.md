...
2. `RGM-84L Harpoon Block II`，原版沒有 `usn_rgm-84l`；模組已新增 `rocn_rgm-84l`，並沿用 `usn_rgm-84d` 外觀資源。
3. `S-70C(M)-1/2` 反潛直升機，原版沒有此航空器；模組已新增 `roc_s-70cm` 最小可行 placeholder，並沿用 `usn_sh-2f` 外觀資源。
...
| RGM-84L Harpoon Block II | 對艦飛彈，公開資料常見記載隨艦移交 32 枚 | 沒有 `usn_rgm-84l`；有 `usn_rgm-84a/c/d` | 已新增 `rocn_rgm-84l`，沿用 `usn_rgm-84d` 外觀 | 已處理 |
| S-70C(M)-1/2 反潛直升機 | ROCN 反潛直升機，基隆級可搭載使用 | 沒有 S-70C/MH-60/SH-60；有 `usn_sh-2f`、`usn_sh-3h` 等 | 已新增 `roc_s-70cm` placeholder，沿用 `usn_sh-2f` 外觀 | 第二階段調校 |
...
| Mk 32 / Mk 46 Mod 5 輕型魚雷 | 近距離反潛魚雷 | 有 `usn_mk46_mod5_ship` | 已使用 `usn_mk46_mod5_ship` | 已足夠 |
...
2. 已完成：複製 `usn_rgm-84d` 做 `rocn_rgm-84l`，調整為 Harpoon Block II 的顯示名稱、對地固定目標能力與必要參數。
3. 已完成最小可行版：`roc_s-70cm` 以 S-70C(M) 名稱與 ROCN 顯示文字進入基隆級艦載機組，外觀暫用 `usn_sh-2f`；性能與武器調校留待第二階段。
